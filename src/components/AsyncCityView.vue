...
const getWeatherData = async () => {
  try {
    const weatherData = await axios.get(
      `https://api.openweathermap.org/data/3.0/onecall?lat=${route.query.lat}&lon=${route.query.lng}&appid=3ad40b7187ddb678858a812b30c2dbd0&units=imperial`
    );
    ...
    weatherData.data.hourly.forEach((hour: HourlyWeatherData) => {
...
const removeCity = () => {
  ...
  const cities = savedCitiesData ? JSON.parse(savedCitiesData) : [];
  const updatedCities = cities.filter(
    (city: City) => city.id !== route.query.id
  );
